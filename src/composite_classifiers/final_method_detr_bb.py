...
"""#########################                                    FINAL METHOD                               #########################"""
"""#########################       DETR 0.3 & 0.5  -->  DETR 0.01 & 0.01  -->  CNN BOUNDING BOXED          #########################"""
...
csv_path = "./final_method_results/final_method_bb_and_whole_prediction_split_2.csv"
detr_annotation_file_path = "./detr_temp_annotation.txt"
...
def create_detr_annotation_file(detections, image_file_name): # creates a temp. annotation file with detr detections for cnn bb
    with open(detr_annotation_file_path, 'w') as file:
...
for root, dirs, files in os.walk(image_directory_path):
    ...
    for file in files:     
        ...
        image = cv2.imread(image_path) # actual image
        ...
        if csv_image_result[-1] == "NOT DETECTED":
            detections_lower_threshold = detr_predict(model, image, 0.01, 0.01)
            create_detr_annotation_file(detections_lower_threshold, file)

        ###################################  CNN PREDICTION  ###################################

            cnn_bb_prediction = cnn_predict(img_file=image_path, ann_file=detr_annotation_file_path, model=CNN_BB_MODEL, type='detr')
            bb_folder = os.listdir(bb_images_path)    
            for bb_image in bb_folder: # delete bb images
                os.remove(bb_images_path+"//"+bb_image)

            if cnn_bb_prediction == 0:
                csv_image_result[-1] = "CNN BB - REAL CANNABIS"
            elif cnn_bb_prediction == 1:
                csv_image_result[-1] = "CNN BB - SYNTHETIC CANNABIS"
            else: 
