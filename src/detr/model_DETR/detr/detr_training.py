...
# labels = ["{} {:.2f}".format(model.config.id2label[5], 0.8339188)]

# labels = [
#    "{} {:.2f}".format(model.config.id2label[class_id], confidence)
...
DIRECTORY = r'D:\Users Data\arthurSoussan\Desktop\yolov4\darknet\data\obj' #folder with all images (jpg, txt)
...
TRAIN_DATASET = CocoDetection(
    image_directory_path=r'D:\Users Data\arthurSoussan\Desktop\detr\final_sets\train_split_2',
    image_processor=image_processor,
    train=True)
VAL_DATASET = CocoDetection(
    image_directory_path=r'D:\Users Data\arthurSoussan\Desktop\detr\final_sets\val_split_2',
    image_processor=image_processor,
    train=False)
TEST_DATASET = CocoDetection(
    image_directory_path=r'D:\Users Data\arthurSoussan\Desktop\detr\final_sets\test_split_2',
    image_processor=image_processor,
    train=False)
...
"""
# ANNOTATIONS ON SINGLE IMAGE - DIDN'T WORKED PROPERLY
# select random image
...
if annotations:
    # annotate
    detections = sv.DetectionDataset.from_coco(images_directory_path=TRAIN_DATASET.root, annotations_path="./simulation_sets/train/annotations.json")
    #detections = sv.DetectionDataset.from_coco(images_directory_path=VAL_DATASET.root, annotations_path="./val/annotations.json")
    #detections = sv.DetectionDataset.from_coco(images_directory_path=image_path, annotations_path="./train/annotations.json")
...
frame = box_annotator.annotate(scene=image, detections=detections.annotations[image['file_name']], labels=labels)
...
# annotate
detections = sv.DetectionDataset.from_coco(images_directory_path="./final_sets/test_split_2",
                                        annotations_path="./final_sets/test_split_2/annotations.json",) # sv.Detections.from_coco_annotations(coco_annotation=annotations)
labels = detections.classes # [f"{id2label[class_id]}" for _, _, class_id, _ in detections]
# print(("labels", labels))

#
# frame = box_annotator.annotate(scene=image.copy(), detections=detections, labels=labels)
# specific_image_detection = None
...
#     # print("Frame", frame)
#     print("Anno- ", anno)
#     if image_file_name in image_path:
#         specific_image_detection = (path, frame, anno)
#         break
#
# if specific_image_detection:
#     print("Specific image detection- ", specific_image_detection)
#     path, frame, anno = specific_image_detection
#     if anno.xyxy.size!=0:
#         class_id = anno.class_id.tolist()
# print("anno = ", anno)
# print("anno.class_id", anno.class_id)
# print("anno.xyxy", anno.xyxy)
# print("anno.xyxy.size", anno.xyxy.size)
# box_annotator = sv.BoxAnnotator()
...
trainer = Trainer(gpus=1, max_epochs=MAX_EPOCHS, gradient_clip_val=0.1, accumulate_grad_batches=8, log_every_n_steps=5, default_root_dir="detr_saved_model")
...
#trainer.save_model("./simulation_sets/detr_results_2") - ERROR
